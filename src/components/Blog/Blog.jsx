...
const BlogListPage = ({ hero }) => {
  ...
  return (
    <div
      className={`bg-gradient-to-br pb-16 from-emerald-50 via-teal-50 to-cyan-100`}
    >
      {!hero ? (
      ...
      <div className="relative max-w-7xl mx-auto px-4">
        {hero && (
          <>
            <button
              onClick={scrollLeft}
              className="hidden md:flex absolute -left-6 top-1/2 -translate-y-1/2 z-10 rounded-full bg-white shadow-lg p-2 hover:bg-gray-100 text-teal-600 pointer-events-auto"
            >
              <ChevronLeft />
            </button>
            <button
              onClick={scrollRight}
              className="hidden md:flex absolute -right-6 top-1/2 -translate-y-1/2 z-10 rounded-full bg-white shadow-lg p-2 hover:bg-gray-100 text-teal-600 pointer-events-auto"
            >
              <ChevronRight />
            </button>
          </>
        ...
        {loading ? (
          ...
          <div
            ref={scrollRef}
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 pt-6 overflow-x-auto scroll-smooth custom-scrollbar"
          >
            {blogs.map((post) => (
              <Link href={`/blog/${post.customUrl}`} key={post._id}>
                <div className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-500 transform hover:-translate-y-2 cursor-pointer h-[420px] flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    {/* <div className="flex items-center justify-between text-teal-600 font-medium group-hover:text-emerald-600 transition-colors text-sm mt-auto">
