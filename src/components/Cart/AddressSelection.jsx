...
import { fetchCartItems, clearUserCart } from "@/redux/thunks/cartThunks";
...
export default function AddressSelection({
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup is optional; only remove if necessary
  ...
  const mappedCartItems =
    cartItems && Array.isArray(cartItems)
      ? cartItems.map((item, index) => ({
          ...
          price: item.productDetails?.saleingPrice || 0,
  ...
  const handleAddNewAddress = async () => {
    ...
    try {
      const response = await addB2cAddressService(addressData);
      // console.log("handleAddNewAddress response", response);
      const savedAddress = response.address || response;
      // Refresh user data to ensure address list is updated
      const userResponse = await getProfileService();
      // console.log("userResponse", userResponse);
      setAddresses(userResponse.user?.address || []);
      setSelectedAddress({
        fullname: savedAddress.fullname || newAddress.fullname,
        street: savedAddress?.street || newAddress?.street,
        city: savedAddress?.city || newAddress?.city,
        state: savedAddress?.state || newAddress?.state,
        pincode: savedAddress?.pincode || newAddress?.pincode,
        country: savedAddress?.country || newAddress?.country,
        addresstype: savedAddress?.addresstype || newAddress?.addresstype,
        otherAddressType:
          savedAddress?.addresstype === "other"
            ? savedAddress?.customType ||
              savedAddress?.otherAddressType ||
              newAddress?.otherAddressType
            : undefined,
  ...
  // Replace handleAddNewBillingAddress with this
  const handleAddNewBillingAddress = async () => {
    ...
    try {
      const response = await addB2cAddressService(addressData);
      // console.log("handleAddNewBillingAddress response", response);
      const savedAddress = response.address || response;
      // Refresh user data to ensure address list is updated
      const userResponse = await getProfileService();
      setAddresses(userResponse.user?.address || []);
      setSelectedBillingAddress({
        fullname: savedAddress.fullname || newBillingAddress.fullname,
        street: savedAddress.street || newBillingAddress.street,
        city: savedAddress.city || newBillingAddress.city,
        state: savedAddress.state || newBillingAddress.state,
        pincode: savedAddress.pincode || newBillingAddress.pincode,
        country: savedAddress.country || newBillingAddress.country,
        addresstype: savedAddress.addresstype || newBillingAddress.addresstype,
        otherAddressType:
          savedAddress.addresstype === "other"
            ? savedAddress.customType ||
              savedAddress.otherAddressType ||
              newBillingAddress.otherAddressType
            : undefined,
        companyName: savedAddress.companyName || newBillingAddress.companyName,
        gstNumber: savedAddress.gstNumber || newBillingAddress.gstNumber,
        _id: savedAddress._id,
      });
      setShowNewBillingAddressForm(false);
      setNewBillingAddress({
      ...
      setAddressError(error.message || "Failed to add billing address");
    } finally {
      setAddressLoading(false);
    }
  };
  ...
  const handleAddressSelection = (address) => {
    setSelectedAddress({
      ...
      street: address.addressinfo?.street || address.street,
      city: address.addressinfo?.city || address.city,
      state: address.addressinfo?.state || address.state,
      pincode: address.addressinfo?.pincode || address.pincode,
      country: address.addressinfo?.country || address.country,
      addresstype: address.addresstype,
      otherAddressType:
        address.addresstype === "other"
          ? address.customType || address.otherAddressType
          : undefined,
      _id: address._id,
    });
  };
